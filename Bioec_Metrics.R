...
Output.Analysis4 <- data.frame()
Productscen <- seq(50000,300000, by = 25000)
Price.add <- seq(10,60,by=.5)
finan <- expand.grid(Productscen,Price.add)


# Scenario 1 Adductor ear hanging Y2
for(i in 1:nrow(finan))
{
  Product <- finan[i,1]
  ScallopAdductor.lbs <- finan[i,2]
  ...
  # Accounts for ear hanging versus grow out at harvest during Fall of Y2 and Y3
  # Also assesses both product types
  
  
  # Scenario 1: Ear Hanging Y2 Harvest (3 full years)
  `Harvest Year` <- 'Y3'
  `Harvest Season` <- 'Fall'
  `Grow Out Method` <- 'Ear Hanging'
  ...
  Output <- data.frame('Product' = Product, 'Harvest Year' = `Harvest Year`, 
                       ...
                       'Price' =  ScallopAdductor.lbs, 'Product Type' = 'Adductor')
  
 Net.Profit <- PL.add[,6]
 Discount <- .12
 
  NPV.Output <- npv(r = Discount, cf=Net.Profit) 

  IRR.Output <- ifelse (PL.add[11,6] > 0,
    irr(cf=Net.Profit),
    NA)
  
  Output <- cbind(Output,NPV.Output,IRR.Output)
 
  
  Output.Analysis4 <- rbind(Output.Analysis4,Output)
  
}

# Scenario 2 Lantern Net Adductor harvest at Y2
...
for(i in 1:nrow(finan))
{
  ...
  # Scenario 1: Lantern Net Y2 Harvest (3 full years)
  `Harvest Year` <- 'Y3'
  `Harvest Season` <- 'Fall'
  `Grow Out Method` <- 'Lantern Net'
...
# Scenarios 3 Y3 ear hanging adductor
...
for(i in 1:nrow(finan))
{
  ...
  # Scenario 1: Ear Hanging Y4 Harvest (4 full years)
  `Harvest Year` <- 'Y4'
  `Harvest Season` <- 'Fall'
  `Grow Out Method` <- 'Ear Hanging'
...
# Scenario 4 Lantern Net Adductor harvest at Y3
...
for(i in 1:nrow(finan))
{
  ...
  # Scenario 1: Lantern Net Y3 Harvest (4 full years)
  `Harvest Year` <- 'Y4'
  `Harvest Season` <- 'Fall'
  `Grow Out Method` <- 'Lantern Net'
...
Productscen <- seq(50000,300000, by = 25000)
Price.whole <- seq(1.00,4.00,by=.1)
finan <- expand.grid(Productscen,Price.whole)
...
for(i in 1:nrow(finan))
{
  ...
  Output <- data.frame('Product' = Product, 'Harvest Year' = `Harvest Year`, 
                       'Harvest Season' = `Harvest Season`, 'Grow Out Method' = `Grow Out Method`,
                       'Price' =  Whole.Scallop.Price, 'Product Type' = 'Whole')
  
  Net.Profit <- PL.Whole[,6]
  Discount <- .12
  
  NPV.Output <- npv(r = Discount, cf=Net.Profit) 
  
  IRR.Output <- ifelse (PL.Whole[11,6] > 0,
                        irr(cf=Net.Profit),
                        NA)
...
write.csv(Output.Analysis4, 'Output_Analysis_4.csv')