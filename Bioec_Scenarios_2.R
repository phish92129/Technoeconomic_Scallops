...
Output.Analysis2 <- data.frame()
Product.s <- seq(50000,600000, by = 10000)
HYear <- c('2-3 Years','3-4 Years')
HSeason <- c('November-January','August-October','February-April','May-July')
GOMethod <- c('Ear Hanging')
Scen1 <- expand.grid(Product.s,HYear,HSeason,GOMethod)

HYear <- '4 Years (August-October only)'
HSeason <- 'August-October'
Scen2 <- expand.grid(Product.s,HYear,HSeason,GOMethod)

Scen1 <- rbind(Scen1,Scen2)
...
# Equipment, Labor, Fuel, and Maintenance tables + Primary and secondary inputs and Pane2 contents
  ...
  Output.Analysis2 <- rbind(Output.Analysis2,Output)
  
}

Output.Analysis2 <- subset(Output.Analysis2, Sea.Days <= 234)

write.csv(Output.Analysis2, 'Output_Analysis_2.csv')

####### Scenario 2 visuals

Output.Analysis2 <- subset(Output.Analysis2, Grow.Out.Method == 'Ear Hanging')
Output.Analysis2$Grow.Out.Method[Output.Analysis2$Grow.Out.Method == 'Ear Hanging'] <- 'Ear-Hanging'
Output.Analysis2$Harvest.Timing[Output.Analysis2$Harvest.Year == "Y2"&Output.Analysis2$Harvest.Season == "Fall"] <- "2.00-2.25 years old"
Output.Analysis2$Harvest.Timing[Output.Analysis2$Harvest.Year == "Y2"&Output.Analysis2$Harvest.Season == "Winter"] <- "2.25-2.50 years old"
Output.Analysis2$Harvest.Timing[Output.Analysis2$Harvest.Year == "Y2"&Output.Analysis2$Harvest.Season == "Spring"] <- "2.50-2.75 years old"
Output.Analysis2$Harvest.Timing[Output.Analysis2$Harvest.Year == "Y2"&Output.Analysis2$Harvest.Season == "Summer"] <- "2.75-3.00 years old"
Output.Analysis2$Harvest.Timing[Output.Analysis2$Harvest.Year == "Y3"&Output.Analysis2$Harvest.Season == "Fall"] <- "3.00-3.25 years old"
Output.Analysis2$Harvest.Timing[Output.Analysis2$Harvest.Year == "Y3"&Output.Analysis2$Harvest.Season == "Winter"] <- "3.25-3.50 years old"
Output.Analysis2$Harvest.Timing[Output.Analysis2$Harvest.Year == "Y3"&Output.Analysis2$Harvest.Season == "Spring"] <- "3.50-3.75 years old"
Output.Analysis2$Harvest.Timing[Output.Analysis2$Harvest.Year == "Y3"&Output.Analysis2$Harvest.Season == "Summer"] <- "3.75-4.00 years old"
Output.Analysis2$Harvest.Timing[Output.Analysis2$Harvest.Year == "Y4"&Output.Analysis2$Harvest.Season == "Fall"] <- "4.00-4.25 years old"

Output.Analysis2$BEAkg <- Output.Analysis2$`Break Even Price (Adductor)`/2.205


Output.Max <- Output.Analysis2 %>% 
  group_by(Harvest.Timing) %>% 
  top_n(1, Sea.Days)

Output.Max$Cat <- 'Maximum Production Unit'

Output.300k <- subset(Output.Analysis2, Product == 300000)

Output.300k$Cat <- 'Standardized Production Unit (300k)'

Output.plot <- rbind(Output.Max,Output.300k)
Output.plot$Cat <- as.factor(Output.plot$Cat)

Output.plot$Harvest.Timing <- factor(Output.plot$Harvest.Timing , 
      levels = c( "2.00-2.25 years old","2.25-2.50 years old","2.50-2.75 years old","2.75-3.00 years old",
                  "3.00-3.25 years old","3.25-3.50 years old","3.50-3.75 years old","3.75-4.00 years old", 
                  "4.00-4.25 years old"))

Change <- c('0','0','+','+','+','+','+','+','+','+','+','+','+','-','+','+','-','-')
C <- as.data.frame(Change)

Output.plot <- Output.plot %>% 
  arrange(Harvest.Timing)
 

Output.plot <- cbind(Output.plot,C)
...
ggplot(Output.plot, aes(x=Harvest.Timing, y = `Break Even Price (Whole Scallop)`, fill = Cat))  + 
  geom_bar(stat='identity', position=position_dodge())+
  geom_text(aes(label=Product), vjust=-.5,position=position_dodge(0.9)) +
  theme_minimal() +
  scale_fill_manual(values = c('#5DBB63','#0492C2'))+
  ...
  ggtitle("") +
  labs(fill = 'Production Unit Category') +
  ylab(bquote('Whole Scallop BE10 '(USD~Scallop^-1)))+
  scale_x_discrete(name = 'Harvest Timing', 
                   labels = c('2.0-2.25\nFall', '2.25-2.50\nWinter', '2.50-2.75\nSpring',
                              '2.75-3.00\nSummer','3.00-3.25\nFall','3.25-3.50\nWinter',
                              '3.50-3.75\nSpring','3.75-4.00\nSummer','4.00-4.25\nFall'))+
...
ggplot(subset(Output.Max, `Adductor Count per lb`<30), aes(x=Harvest.Timing, y = BEAkg))  + 
  ...
  theme(panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank()) + #remove gridlines
  ggtitle("") +
  ylab(bquote('Adductor Muscle BE10 '(USD~kg^-1)))+
  scale_x_discrete(name = 'Harvest Timing', 
                     labels = c('2.75-3.00\nSummer','3.00-3.25\nFall','3.25-3.50\nWinter',
                                '3.50-3.75\nSpring','3.75-4.00\nSummer','4.00-4.25\nFall'))+
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1),
...
ggplot(subset(Output.plot, `Adductor Count per lb`<30), aes(x=Harvest.Timing, y = BEAkg, fill = Cat))  + 
  geom_bar(stat='identity', position=position_dodge())+
  geom_text(aes(label=Product), vjust=-.5,position=position_dodge(0.9)) +
  geom_text(aes(label=Change), size = 8, vjust=1.2,position=position_dodge(0.9))+
  theme_minimal() +
  scale_fill_manual(values = c('#5DBB63','#0492C2'))+
  theme(panel.grid.major.x=element_blank(), panel.grid.major.y=element_blank()) + #remove gridlines
  theme(panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank()) + #remove gridlines
